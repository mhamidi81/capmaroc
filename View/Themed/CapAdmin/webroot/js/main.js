...
App.startPageLoading = function(message) {
    $('body').append('<div class="loading-message loading-message-boxed page-loading"><img style="" src="' + Croogo.basePath+'theme/CapAdmin/img/ajax/loading-spinner-grey.gif" align=""><span>&nbsp;&nbsp;' + (message ? message : 'Operation en cours') + '</span></div>');
};
App.stopPageLoading = function() {
    $('div.page-loading').remove();
};

App.confirm = function(message, on_success_callback) {
    bootbox.dialog({
        message: message,
        title: "Confirmation",
        className: "bootbox-custom-confirm",
        buttons: {
            success: {
                label: "Oui",
...
App.alert = function(message, on_success_callback) {
	bootbox.alert({
        message: message,
        className: "bootbox-custom-alert",
        buttons: {
            ok: {
                label: "ok",
...
var Main = function() {
	...
	var datepicker = function() {
		/*var dateTag = '<p class="input-group input-append datepicker date">'+
			'<input type="text" class="form-control datepicker" id = "id_placeholder" name = "name_placeholder">'+
			...
			'</span>'+
		'</p>';
		var datepickers = $(document).find('input.datetimepicker').each(function(){
			var id = $(this).attr('id');
			var name = $(this).attr('name');
			dateTag = dateTag.replace('name_placeholder', name).replace('id_placeholder', id);
			$(this).replaceWith(dateTag);
		});*/
	...
	var settingsHandler = function() {
		...
		$('input[name="setting-theme"]').change(function() {
			...
			$('#skin_color').attr("href", Croogo.basePath+"theme/CapAdmin/css/themes/" + selectedTheme + ".css");
		...
		function switchLogo(theme) {
			switch (theme) {
				...
				case "theme-5":
				case "theme-6":
					$(".navbar-brand img").attr("src", Croogo.basePath+"theme/CapAdmin/img/logo2.png");
					break;

				default:
					$(".navbar-brand img").attr("src", Croogo.basePath+"theme/CapAdmin/img/logo.png");
					break;
			};
		};
		function defaultSetting() {
			...
			$('#skin_color').attr("href", Croogo.basePath+"theme/CapAdmin/css/themes/theme-1.css");
			$(".navbar-brand img").attr("src", Croogo.basePath+"theme/CapAdmin/img/logo.png");
